...
function getUserDashboard(user) {
    ...
    let displayName = user.profile != null ? user.profile.name || "Guest" : "Guest";
    // 2. Xử lý hiển thị ảnh đại diện (Lỗi: Sập hệ thống nếu user.profile null)
    let avatarImg = user.profile != null ? user.profile.avatar || "default-avatar.png" : "default-avatar.png";
    // 3. Xử lý hiển thị nút quản trị (Rất rối rắm)
    let adminPanel = user.isLoggedIn ? (user.role === "ADMIN" && "--- OPEN ADMIN PANEL ---") || "No Access" : "Please Login";
...
// getUserDashboard(user2); // Bỏ comment để thấy lỗi sập hệ thống
